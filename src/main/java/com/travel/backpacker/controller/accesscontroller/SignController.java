...
public class SignController {
    ...
    @PostMapping("/login")
    public HttpEntity login(@ModelAttribute("wrapper") UserWrapper wrapper, BindingResult bindingResult, @RequestBody LoginData loginData, BindingResult bindingResult1, @RequestHeader(name = "Packer-Platform", required = false) String originPlatform) {
        Operation<UnknownUser> operation = unknownUserOperationFactory.getLoginOperation(loginData, wrapper, originPlatform);
        if (bindingResult1.hasErrors()) {
            //do something
        }
        String metricJson = String.format("{\"time\": %d, \"event\": \"metric\", \"fields\": {\"metric_name:%s\": %f}}", System.currentTimeMillis() / 1000, "test.metric", 0.5);
    ...
    public HttpEntity registerUser(@ModelAttribute("wrapper") UserWrapper wrapper, @Valid @RequestBody RPassenger rPassenger, @RequestHeader(name = "Packer-Platform", required = false) String originPlatform) {
    ...
    public HttpEntity registerAdmin(@ModelAttribute("wrapper") UserWrapper wrapper, @Valid @RequestBody RAdmin rAdmin, @RequestHeader(name = "Packer-Platform", required = false) String originPlatform) {
    ...
    @GetMapping("/get/{id}")
    public HttpEntity getUser(@ModelAttribute("wrapper") UserWrapper userWrapper, @Pattern(regexp = "[0-9]{6}", message = "Invalid user Id") @PathVariable("id") long id) {
